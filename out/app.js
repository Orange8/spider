...
const go = () => __awaiter(this, void 0, void 0, function* () {
    const res = yield api_1.remote_get('https://www.douban.com/group/szsh/discussion?start=0', {});
    console.log(res);
    // 加载网页
    ...
    titles.map((item, index) => {
        utils_1.db('insert into info_list(title,url) values(?,?)', [item, urls[index]], (err, data) => {
            if (data) {
                console.log('提交数据成功！！');
            }
            if (err) {
